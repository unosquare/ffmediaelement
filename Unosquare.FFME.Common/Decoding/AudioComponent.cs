                ...
                fixed (AVFilterContext** sink = &SinkFilter)
                {
                    ...
                    if (result != 0)
                    {
                        throw new MediaContainerException(
                            $"{nameof(ffmpeg.avfilter_graph_create_filter)} (audio_buffer) failed. Error {result}: {FFInterop.DecodeFFmpegMessage(result)}");
                    }
                    ...
                    {
                        throw new MediaContainerException(
                            $"{nameof(ffmpeg.avfilter_graph_create_filter)} (audio_buffersink) failed. Error {result}: {FFInterop.DecodeFFmpegMessage(result)}");
                    }
                }
