        ...
        private void InitializeFilterGraph(AVFrame* frame)
        {
            ...
            try
            {
                ...
                fixed (AVFilterContext** sink = &SinkFilter)
                {
                    ...
                    if (result != 0)
                        throw new MediaContainerException($"{nameof(ffmpeg.avfilter_graph_create_filter)} (buffer) failed. Error {result}: {FFInterop.DecodeFFmpegMessage(result)}");
                    ...
                    if (result != 0)
                        throw new MediaContainerException($"{nameof(ffmpeg.avfilter_graph_create_filter)} (buffersink) failed. Error {result}: {FFInterop.DecodeFFmpegMessage(result)}");
                ...
                if (string.IsNullOrWhiteSpace(FilterString))
                {
                    ...
                    if (result != 0)
                        throw new MediaContainerException($"{nameof(ffmpeg.avfilter_link)} failed. Error {result}: {FFInterop.DecodeFFmpegMessage(result)}");
                }
                else
                {
                    ...
                    if (result != 0)
                        throw new MediaContainerException($"{nameof(ffmpeg.avfilter_graph_parse)} failed. Error {result}: {FFInterop.DecodeFFmpegMessage(result)}");
                ...
                if (result != 0)
                    throw new MediaContainerException($"{nameof(ffmpeg.avfilter_graph_config)} failed. Error {result}: {FFInterop.DecodeFFmpegMessage(result)}");
            }
            catch (Exception ex)
            {
